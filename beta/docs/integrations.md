...
The picker close strip and native dialog share explicit dimensions, including after viewport changes. Creating a file in a shared drive also requires deploying the `github-worker` change that supplies `supportsAllDrives=true` for folder validation and uploads; a frontend-only Pages deployment does not update that Worker.
