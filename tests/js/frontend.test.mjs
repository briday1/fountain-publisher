...
test("source word wrap defaults on and preserves logical line numbers", async () => {
  ...
  assert.match(app, /number\.className = "line-number";/);
  assert.match(app, /if \(number\.style\.top !== top\) number\.style\.top = top;/);
  ...
  assert.match(app, /if \(gutter\.scrollTop !== scrollTop\) gutter\.scrollTop = scrollTop;/);
  assert.match(app, /node\.dataset\.sourceLine = String\(index\);/);
  assert.match(app, /node\.innerHTML = value;/);
