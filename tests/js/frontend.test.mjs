...
test("character analytics supports a scrollable timeline, PNG save, and CSV copy", async () => {
  ...
  assert.match(html, /id="character-analytics-full"[^>]*>Full timeline</);
  ...
  assert.match(app, /character-analytics-full"\)\.addEventListener[\s\S]*characterAnalyticsDocumentGroup\(\)/);
  assert.match(app, /y < 28[\s\S]*characterAnalyticsActGroups\(\)\.find/);
